...
import {algoritm} from "../../../Helper/Helper";
...
const Wall = ({location}: any) => {
    ...
    React.useEffect(() => {
        let check: string = "";
        walls.length > 0 && walls.map((item: IWalls) => {
    ...
    return (
        <div className="page-wrapper">
            ...
            {walls.length > 0 ? <ul className="wall-item-container">
                {walls.map((item: IWalls, index: number) => (
                    <li className="wall-item" key={index}>
                        {index + 1}
                        <div>
                            <i onClick={() => openModalHandler(index)} className="icon-pencil pen"></i>
                            <i onClick={() => wallDeleteHandler(index)} className="icon-bin bin"></i>
                            {item.type && item.width && item.height ? <i className="icon-checkmark check"></i> : null}
                        </div>
                    </li>
                ))}
            </ul> : <p>Yoy must add wall</p>}
            {isRedy.split("").length === walls.length && walls.length > 0 ?
                <button onClick={sendHandler}>Send</button> : null}
            <div>
                {transaction.map(({item, key, props: animation}) => (
                    item &&
                    <Modal
                        key={key}
                        state={input}
